    ...
    $('.fileupload:file').change(function () {
        ...
        if (file.name.length < 1) {} else if (file.size > 209715200) {
            ...
            $.ajax({
                ...
                success: function success(data) {
                    if ($('.txtfoto').length > 0) {
                        fileinput.closest('div.controlupload').find('.txtfoto').css({ "color": "peru", "border": "2px solid blue" }).val(data.filename);
                        fileinput.parent().parent().parent().find('img.imgfoto').attr('src', fileinput_path + data.filename);
                    }
                },
                error: errorHandler = function errorHandler() {
...
(function ($, window, document) {
    ...
    var table_agenda = $('#table_agenda').DataTable({
        ...
        columns: [{
            ...
            "searchable": false,
            "data": null,
            "defaultContent": '<div class="input-group margin">' + '<div class="input-group-btn">' + '<button type="button" class="btn btn-default btn-xs dropdown-toggle" data-toggle="dropdown">' + '<span class="fa fa-caret-down ico-caret-down"></span></button>' + '<ul class="dropdown-menu">' + '<li><a class="edit">Edit</a></li>' + '<li data-title="Hapus Agenda" data-message="Hapus Agenda ??"><a class="hapus">Hapus</a></li>' + '</ul>' + '</div>' + '</div>'

        }, { data: 'judul_agenda', name: 'judul_agenda' }, { data: 'tempat', name: 'tempat' }, { data: 'waktu', name: 'waktu' }]
    });
...
(function ($, window, document) {
    ...
    if ($(".tinymce_sikko").length > 0) {
        tinymce.init({
            ...
            plugins: ["advlist autolink link image lists charmap print preview hr anchor pagebreak", "searchreplace wordcount media nonbreaking spellchecker", "table contextmenu directionality emoticons paste textcolor responsivefilemanager code "],
            relative_urls: false,
